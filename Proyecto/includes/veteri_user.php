    ...
    <div class="tarjetas">
        <div class="tarjeta-container">
            ...
            <div class="tarjeta-content notificaciones">
                ...
                <?php } ?>
                <?php
                $query = "SELECT c.Id_Cita, c.Fecha_Cita, m.Nombre_Mascota FROM citas AS c, mascotas AS m WHERE c.Id_Veterinaio = $idVeterinario AND c.Id_EstadoCita = 1 AND c.Id_Mascota = m.Id_Mascota
        ...
        <div class="tarjeta-container">
            ...
            <?php
                $query = "SELECT m.Nombre_Mascota, a.Nombre, m.Id_Especie FROM citas AS c, mascotas AS m, clientes AS a WHERE c.Id_Veterinario = 1 AND c.Id_EstadoCita = 1 AND c.Id_Mascota = m.Id_Mascota And m.Id_Cliente = a.Id_Cliente";
                $resultado = mysqli_query($conn, $query);
                while ($row = mysqli_fetch_array($resultado)) { 
                $nombreMascota = $row['Nombre_Mascota'];
                $nombreCliente = $row['Nombre'];
                $especie = $row['Id_Especie'];
                if (count($nombre_Mascota) == 0) { ?>
                ...
                <?php } ?>
                <?php for ($i = 0; $i < count($nombreMascota); $i++) { ?>
